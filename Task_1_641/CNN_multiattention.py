...
LEARNING_RATE = 0.005  # on overfitting
NUM_EPOCHS = 25
L2_REG = 0.001  # on overfitting
...
class MultiHeadAttention1D(nn.Module):
    def __init__(self, num_filters, num_heads):
        ...
        self.qkv_proj = nn.Linear(num_filters, num_filters * 3)
        # Output projection
        self.out_proj = nn.Linear(num_filters, num_filters)

    def forward(self, x):
        # x: (batch, num_filters, seq_len)
        x_t = x.transpose(1, 2)  # (batch, seq_len, num_filters)
        batch_size, seq_len, num_filters = x_t.shape

        # Project to queries, keys, values
        qkv = self.qkv_proj(x_t)  # (batch, seq_len, num_filters*3)
        q, k, v = torch.chunk(qkv, 3, dim=-1)  # each is (batch, seq_len, num_filters)
        ...
        # Collapse sequence: weighted sum over seq_len (could use mean, but follow attention)
        # We'll use the "[CLS]"-like approach: take the weighted sum over input positions for each head, then flatten.
        # Alternatively, take the mean over the sequence output (as in previous answer).
        attn_output = attn_output.mean(dim=2)  # (batch, num_heads, head_dim)

        # Concatenate heads
        attn_output = attn_output.reshape(batch_size, -1)  # (batch, num_filters)
        out = self.out_proj(attn_output)  # (batch, num_filters)
        return out

class CNN(nn.Module):

    def __init__(self, vocab_size, embed_dim, num_filters, filter_sizes,
                 num_classes, num_heads=4, dropout_rate=0.5):
        ...
        self.attentions = nn.ModuleList([
            MultiHeadAttention1D(num_filters, num_heads) for _ in filter_sizes
        ])
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.convs):
            conv_out = F.relu(conv(embedded))  # (batch, num_filters, conv_seq_len)
            attn_out = self.attentions[i](conv_out)  # (batch, num_filters)
            conv_outputs.append(attn_out)
